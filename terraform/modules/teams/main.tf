...
resource "google_project_iam_member" "full_stack_dev" {
  for_each = length(var.full_stack_devs) > 0 ? toset(var.full_stack_devs) : {}
...
resource "google_project_iam_member" "backend_dev" {
  for_each = length(var.backend_devs) > 0 ? toset(var.backend_devs) : {}
...
resource "google_project_iam_member" "frontend_dev" {
  for_each = length(var.frontend_devs) > 0 ? toset(var.frontend_devs) : {}
